...
ROOMS = {}  # redis or a db in the future maybe
...
# --- NEW: signaling relay helpers ---
async def relay_json(room: Room, from_uid: str, payload: dict):
    """Forward JSON signaling messages to the other peer(s) in the room."""
...
async def handle_ws(ws, rid, uid):
    ...
    # Start the music loop once someone is in the room
    start_room(room)

    # Start binary audio sender
    sender = asyncio.create_task(sender_loop(client))

    # Notify others that a peer joined (helps decide who creates offer)
    try:
        await relay_json(
            room, uid, {"type": "peer-joined", "uid": uid, "peers": room_size(room)}
        )
    except:
        pass

    # Also tell the joining peer the current peer count (optional)
    try:
        await ws.send_text(json.dumps({"type": "peer-count", "peers": room_size(room)}))
    except:
        pass
    ...
    try:
        ...
        try:
            await relay_json(
                room, uid, {"type": "peer-left", "uid": uid, "peers": room_size(room)}
            )
        except:
            pass
        if not room.clients:
